 ...
 * @author: Tianchan Dong and Damiche Ines
...
contract RSA {
    ...
    // Function: modular exponentiation (base^exp % mod)
    function modExp(uint base, uint exp, uint mod_) internal pure returns (uint) {
        if (mod_ == 1) return 0;
    ...
    function Sign_Message(string calldata data) public view returns (uint) {
        ...
        uint signed = modExp(digest, ENCRYPT, MOD);
    ...
    function verify(string calldata data) public view returns (bool) {
        ...
        uint verify_with_public_key = modExp(signed_message, DECRYPT, MOD);
